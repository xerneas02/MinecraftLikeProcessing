...
{  
  fullScreen(P3D);
  img = loadImage("texture/block.png");
...
void draw2DInfo()
{
  ...
  textAlign(LEFT, TOP);
  textSize(24);
  text("Fps: "+nf(frameRate, 0, 1), 0, 0, 0);
  text("x: "+nf(player.pos.x, 0, 1), 0, 30, 0);
  text("y: "+nf(player.pos.y, 0, 1), 0, 60, 0);
  text("z: "+nf(player.pos.z, 0, 1), 0, 90, 0);
  text("chunckX: "+ player.chunck.x, 0, 120, 0);
  text("chunckZ: "+ player.chunck.y, 0, 150, 0);
  beginShape();          
  texture(img);
